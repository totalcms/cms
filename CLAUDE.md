...
- **Sidebar menu** lives in `resources/docs/menu.php` (shared by `AdminDocsAction` and `bin/build-docs-index.php`). 14 top-level groups: Get Started, Concepts, Admin, Collections, Schemas, Fields, Forms, Notifications, Site Builder, Templates (Twig), Auth, APIs, Extensions & CLI, Operations. Adding a new doc page = add a `{title, path}` entry to the appropriate group. Fields, Templates, and Extensions & CLI use nested subgroups (the last via mixed `sub` + `groups`); everything else is flat.
...
- **Search index**: regenerated by `bin/build-docs-index.php`, written to `resources/docs/search-index.json`. Run after adding/renaming doc pages.
