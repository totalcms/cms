...
class totalpreview
{
	...
	public function oembed($content)
	{
		...
		foreach ($macros[0] as $macro) {
			...
			try {
				...
				$class = strtolower("$info->type $info->providerName");
				$code = "<div class=\"oembed $class\">$info->code</div>";
			}
			catch (Exception $e) {
	...
	{
		if ($this->type === 'text') return $this->totaltext_replace($buffer,$prefix);
		return $buffer;
	}
	...
	private function cmsGalleryImageFeaturedAlt($macro,$slug,$buffer)
	{
		...
		usort($images, function($a,$b){ return $b['featured'] - $a['featured']; });
		$alt = $images[0]['alt'];
	...
	private function cmsDataStoreDownload($macro,$filename,$buffer)
	{
		...
		$download = $this->baseurl."rw_common/plugins/stacks/dynamics/totaldownload.php?type=datastore&slug=$slug";
	...
	{
		$info = pathinfo($filename);
		$download = $this->baseurl."rw_common/plugins/stacks/dynamics/totaldownload.php?type=file&slug=".$info['filename']."&ext=".$info['extension'];
		return str_replace($macro, $download, $buffer);
	}
	...
	{
		return str_replace($macro, date('F',$post->timestamp), $buffer);
	}
	...
	{
		return str_replace($macro, date('l',$post->timestamp), $buffer);
	}
	...
	{
		if (!$this->checkBlogImages($post)) return str_replace($macro, '', $buffer);
	...
	{
		$feed_url = $this->baseurl."rw_common/plugins/stacks/dynamics/totalapi.php?type=blog&slug=$slug&filter=".json_encode($filter);
	...
	{
		$feed_url = $this->baseurl."rw_common/plugins/stacks/dynamics/totalapi.php?type=blog&slug=$slug&permalink=$permalink";
		$results = $this->query($feed_url);
	...
	{
		$feed_url = $this->baseurl."rw_common/plugins/stacks/dynamics/totalapi.php?type=feed&slug=$slug";
		$results = $this->query($feed_url);
	...
	{
		$gallery_url = $this->baseurl."rw_common/plugins/stacks/dynamics/totalapi.php?type=gallery&slug=$slug";
	...
	{
		$depot_url = $this->baseurl."rw_common/plugins/stacks/dynamics/totalapi.php?type=depot&slug=$slug";
