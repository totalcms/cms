...
## [3.5.0-rc.21] - 2026-08-25

### Fixed

- **Creating an OAuth client from the admin always failed with a 404**: The form passed its route as `/api/oauth-clients`, but a `totalform` route is relative to the form's `data-api`, which already carries the `/api` prefix — so every submission posted to `/api/api/oauth-clients` and nothing could ever be created. It was broken on every install, root and subfolder alike, from the moment the form shipped, and went unnoticed because it is the rare path: MCP clients self-register through RFC 7591 dynamic registration and never touch this screen, so only a hand-registered integration hit it. Nothing about the 404 pointed at a URL — it reads as a routing or permissions problem, and the endpoint itself was fine all along. A test now walks the templates for any literal form route carrying its own `/api`, because the failure is invisible until someone opens the network tab
- **Overrides returned from `config/tcms.php` were silently discarded on zip and Stacks installs**: The Composer branch of the settings loader merged the array the file returns; the zip branch simply `require`d the file and threw the return value away. So an operator following the documentation — which told them to set `api` there — got no error, no warning, and no effect, then reasonably concluded the setting was broken rather than the file being the wrong one. Both styles are now honoured: a file with no return statement still yields `1` from `require`, so the historical `$settings[...] = …` assignment form that zip installs have always used keeps working alongside the returned array everything else documents. The documentation now also states plainly which override file applies to which install type, including the document-root `tcms.php` that zip and Stacks installs additionally read
- **A Claude connector on a subfolder install completed OAuth and then failed every call**: The grant appeared under **OAuth Grants**, the token was valid and unexpired, and every subsequent MCP request came back `401` — a signature that looks like a token problem and is not one. For an issuer with a path component, RFC 8414 §3.1 puts the authorization-server metadata at `/.well-known/oauth-authorization-server/<issuer-path>`, which on a subfolder install with the setup wizard's root catch-all rewrite arrives at the *domain root*. The mount prefix is worked out per request by cross-checking `SCRIPT_NAME` against the request path, and a root-shaped request has no subpath to match, so it resolved to empty and the document advertised the bare-host issuer. The client had just been handed the subpath issuer by the protected-resource document, refused the mismatch, and stopped — correctly, per the spec. That URL now answers for the issuer that was actually queried, the way the protected-resource endpoint already echoed the resource it was asked about. A base path the install does not serve is never echoed back — the install's own document is returned instead, since every endpoint is built onto the issuer and reflecting an arbitrary path would advertise endpoints pointing anywhere on the host; an explicitly configured `oauth.jwtIssuer` stays authoritative. That URL keeps answering rather than 404ing for an unknown path, because not every client derives it from the issuer the way §3.1 specifies — some append the protected resource path instead, and on a root install, whose issuer has no path component at all, a 404 there reads to the client as "this server has no OAuth". **No configuration is needed for any of this** — both base paths are now internally consistent, and a client stays on whichever shape it discovered through. Accordingly the **Test connection** panel's "Single discovery authority" check no longer warns when the two shapes advertise different endpoints: that is now reported as information rather than flagged as a fault, because a warn means an operator problem was found and there is no longer one to find. Pinning `api` remains available for anyone who would rather the two shapes advertise identical endpoints, but it is a preference, not a repair
- **The OAuth activity log had never recorded a single token issuance**: `tokenIssued` and `tokenRefreshed` were guarded on `isset($payload['scope'])`, a key league's `BearerTokenResponse` does not set — its body carries `token_type`, `expires_in`, `access_token` and optionally `refresh_token`, and nothing else. So the guard was false on every install, on every exchange, since the feature shipped, and a log that documents itself as the authoritative record of OAuth activity silently omitted the one event that matters most. The failure mode is worse than a missing line: during a live support case the absent entries were read as evidence that token exchange never completed, when the web server's access log showed all thirteen exchanges returning 200. A diagnostic that is quietly empty is worse than one that is absent, because it answers questions confidently and wrongly. The granted scopes are now read from the issued token's own `scopes` claim, decoded from the JWS payload without a key — this is an audit line, not a trust decision, and the token was minted by the same process moments earlier
- **Test connection reported a firewall problem on every install with anonymous access switched off**: The endpoint probe sends no credentials, so a site with Public Access off answers it `401` — the correct response, and the configuration a security-conscious operator would deliberately choose. The check called that a failure and advised looking for maintenance mode or a WAF intercepting the endpoint, which sent a real support case hunting a firewall that did not exist on a server behaving exactly as configured. An unauthenticated `401` now passes when Public Access is off, and the remedy text for a genuine `401` distinguishes the case where anonymous callers *should* have been served
- **The "Bearer auth reaches PHP" check passed without testing anything**: It concluded the header had arrived whenever an invalid token drew a `401` — but on a site with Public Access off a *stripped* header draws `401` too, `login_required` rather than `invalid_token`. The one probe whose entire purpose is detecting a stripped Authorization header therefore certified a live site healthy while OAuth clients on it were authenticating successfully and being treated as anonymous on every subsequent call. It now sends a second, credential-free control request and compares: if the answers are byte-identical the credential made no difference to the outcome, which means it never arrived. That comparison is the only discriminator that holds in every configuration, and the failure detail says plainly that OAuth cannot work until it is resolved — the tokens are valid, but no client can present them
- **A calc field summing a price column returned 11 instead of 11,000**: Reported as broken float maths, and it looks like it — but the arithmetic was never involved. A price field is a masked text input, so its DOM value is the display string `5,000.00`, and `parseFloat()` stops at the group separator: 5 + 6. Below 1000 there is no separator, which is why the same form totalled 500 + 600 correctly and made this read as a rounding bug rather than a parsing one. `calc.js` was the last place in the form system still reading raw inputs — the save path already went through the typed-value collector, and the server recomputes every calc field on save, so **nothing was ever stored wrong**: the total on screen simply was not the total in the database, on an invoice form where the operator has no reason to doubt it. Existing objects need no re-saving. Reading through the typed-value collector also closes a latent second defect in the same method, where a composite field's sub-field could shadow a real deck column of the same name. The computed result now writes through the field's own setter rather than straight to the input, which is the same defect on the write side and accounts for the other half of what the operator saw: the total rendered as `1100` where every other price on the form read `$1,100.00`, because assigning the input value bypasses the mask
- **The two calc engines disagreed about what a field was worth**: A calc expression is evaluated twice — live in the browser as the operator types, and again in PHP on save — and the two had drifted apart. JS coerced with `parseFloat` and PHP with `is_numeric`, so a time field holding `12:30` contributed 12 live and 0 on save, and a text field holding `3 items` contributed 3 and 0. A calc could therefore show one total while typing and store another, with nothing on screen to say so. Both engines now share three tiers: already-numeric, then a display-formatted price, then zero. The second tier fixes a silent zero of its own — calc runs on raw request data, before the price property is built, so a price arriving formatted from a raw API post, a CSV import or a JS-disabled browser evaluated as 0 while the price itself stored correctly. The guard in front of that tier is deliberate and worth knowing: the price normaliser strips every character that is not a digit or separator, which is only safe when the value is already known to be a price, so unguarded it reads a date field as 2026 and a phone number as 555. A visible zero beats a plausible wrong total — 11 was believable enough to take three rounds of customer testing to characterise. The contract now lives in one fixture asserted by both engines, so a coercion changed in one and not the other fails the other's suite. One behaviour change reaches beyond price: a text field holding `3 items` now contributes 0 in the browser rather than 3, matching what the server has always stored

### Enhanced

- **Rejected OAuth token exchanges are recorded**: Previously the audit trail stopped at `consent.granted` and the reason a client failed at the final step lived only in the web server's access log, which an operator on shared hosting usually cannot reach. Failures now log at warning level with the client id, grant type, error type, league's hint and the caller's address
- **Server Info reports the PHP SAPI**: `cgi-fcgi` versus `fpm` decides whether Apache drops the Authorization header before PHP ever sees it, which makes it the first thing worth knowing on any report of authentication silently degrading to anonymous. It was not shown anywhere in the admin

## [3.5.0-rc.20] - 2026-08-22

### Added

- **New trials register a name and a verified email before they start**: The wizard's license step becomes a three-state flow — register, enter the six-digit code sent to that address, then the confirmation screen — and the account step now collects the operator's real name so both fields arrive pre-filled. Until now a trial was minted for whoever asked, with no contact on file, which meant nobody could be told their trial was about to expire and nothing distinguished one evaluator from fifty. Registration is the primary action on that screen, but skipping stays possible throughout, so a customer holding a license key or standing up a dev install is never trapped — and a server error keeps the old error-and-skip screen rather than demanding registration the server cannot process. Existing trials created before this are grandfathered rather than broken: they keep working, and the dashboard carries a nudge to register so expiry reminders can reach someone. The `registered` flag defaults to true and is parsed defensively, so an install talking to an older license server never sees a nag it cannot act on. One incidental change worth knowing: the first admin user's id now comes from the auth schema's `oid` autogen instead of the hardcoded `admin`, so the obvious username is no longer the guaranteed one
- **`tcms object:patch` closes the last gap in the CLI's write surface**: `object:create` only makes new objects and `collection:import` only works in bulk, so changing one field on one object had no supported route from a terminal — the practical result being that it got done by editing JSON on disk, which skips validation, the collection index, and `object.updated` events. An agent over MCP could already patch an object; the operator could not. The command wraps `ObjectPatcher`, so all three come back, and it mirrors the two existing HTTP endpoints rather than inventing a third shape: a bare patch merges at the top level, `--property` merges into that property. That distinction is load-bearing and is shown as a worked example in the command help, because `patchObject` is a shallow merge — patching a structured field from the top level replaces it wholesale, dropping the keys you did not mention. `object:update` is deliberately absent; it is the more dangerous verb and the two existing commands already cover writing a whole object
- **`cms.builder.canonicalUrl()` for absolute page URLs**: `cms.builder.url()` returns a site-relative path and there was no page equivalent of `cms.collection.canonicalObjectUrl()`, so a layout wanting a canonical tag, an `og:url` or a redirect had to assemble the absolute URL itself — which means hard-coding a host into a template, wrong everywhere the site is not production and stale without ever failing loudly. It delegates to `url()`, so the base prefix, `{param}` filling, encoding and the optional collection argument all carry through, and it accepts a page id or a whole page array so a layout can hand over the `page` it already holds. It returns an empty string wherever `url()` does — a missing page, or one with no route — because falling back to a bare domain renders as a link to the homepage, which looks like it works. Both it and `url()` are now documented in the Site Builder Twig reference, which turned out to document neither

### Changed

- **Site Builder pages are identified by their URL rather than their title**: A page title is written for a browser tab — "Total CMS 3 — Your Site. Your Data. Your Terms." — and truncates to noise at sidebar width, while the route is short, unique, and is what actually identifies a page in a router-driven builder. The builder sidebar now labels pages by route with the title moved to the hover tooltip, and the Sync Manager does the same, preferring `route` over `title` when an index entry has one. Titles are neither unique nor stable: localized variants are the obvious case, where two pages both read "About" while living at paths that tell you which is which. Worth knowing this also changes what the sidebar filter matches, since it filters on the rendered label — pages are found by URL now, not by name. Of the syncable collections only `builder-pages` carries a route, so mailer, automations, mcp-prompt and dataviews fall through to title/name/subject exactly as before

### Fixed

- **Concurrent API-key writes could wipe `apikeys.json` entirely**: Production installs were losing every API key, silently, with no correlation to deploys. Validating a key stamped its `lastUsed`, which meant a read-modify-write over the whole file on a path every authenticated request takes. Flysystem's local adapter does pass `LOCK_EX` on write, but that is not enough on its own: `flock` is advisory and the adapter's read is a bare `file_get_contents` with no shared lock, so a reader sails straight through a writer's lock; `file_put_contents` truncates before writing, so there is a real window where the file is empty or partial; and a per-call lock cannot make a two-call read-then-write cycle atomic regardless. A reader landing in that window decoded nothing and got back "this install has no keys" — indistinguishable from the truth — which the next write then persisted over every real key. Two concurrent authenticated requests were enough, which is why it only ever appeared on live sites. Three things change, and all of them matter: an unreadable or malformed file now throws instead of decoding to an empty list, which is the actual fix — silently reinterpreting "I cannot read your secrets" as "you have no secrets" is what turned a transient read into permanent loss; mutations hold an exclusive lock across the whole read-modify-write cycle; and the commit is a write-to-temp-then-rename, so unlocked readers see the old file or the new one, never a torn one. The `lastUsed` stamp is also debounced to 60 seconds — it was the only reason a read path wrote at all, and it is displayed as a coarse "last active" hint, so minute granularity costs nothing and removes almost all the contention
- **The MCP listening stream held every php-fpm worker on a busy site**: The `GET` keepalive stream added in rc.18 pinned a production pool at its ceiling around the clock — 17 MCP clients holding all 20 workers continuously, about 1.9GB, with the box in swap for days before anyone connected the two. The reasoning that called the feature self-limiting was wrong in both halves: the per-IP rate limiter exempts every caller sending a bearer token, which is every OAuth-authenticated MCP client, so those had no bound at all; and it bounds one IP, never the aggregate, so seventeen clients at roughly five workers each is eighty-five workers of demand against a pool of twenty. The steady-state cost per client was also higher than it looked, since a client cycles occupied-then-idle and the old five-second window against a hardcoded two-second retry held 71% of a worker per connected client, forever, for a stream carrying nothing but keepalives. A new `listeningStreamMaxConcurrent` (default 2) caps streams site-wide across all callers — the only bound that reaches OAuth clients and the only one that bounds the total — and over the cap callers get the SDK's ordinary spec-legal `405`, exactly as if the stream were switched off. `listeningStreamSeconds` drops from 5 to 1 and `listeningStreamRetryMs` replaces the hardcoded 2000 (default 15000), which together take a well-behaved client from 71% of a worker to about 6%. A window of `0` is now supported and is the cheapest setting that still passes a scanner's probe: the keepalive and `retry:` are written before the wait loop, so the probe gets a real `200 text/event-stream` with an event in it while the worker returns in milliseconds — and at a zero window the cap is skipped entirely, because there is no occupancy to bound and counting anyway turned it into a global request-rate limit that refused probes with the very `405` the stream was added to prevent
- **A sync push made the receiving site invent random images**: The exporter wrote the literal string `image` as an image field's value, intended as a type marker — but that is byte-identical to the factory-rule syntax the importer honours, so the destination read it back as "generate an image here" and did, on every object with an image field, on every push. Clearing an image at the source could never propagate either, because the payload had no way to express an empty value: it always said `image`, and the destination always obliged. Nobody noticed, because a fabricated image looks like a successful sync. JumpStart carries no binaries, so an image field is not syncable data in either direction — the source cannot send one, and it must not clear one, having no authority over media it never received. The exporter now omits the key entirely, which is the only honest encoding of "this did not travel", and the importer carries the destination's existing image and gallery values forward for any schema-declared media field the payload does not mention. Only absent fields are preserved: a value that is present is an authored factory rule, and starter kits legitimately seed demo content that way, so that path still works. `depot` and `file` fields already normalised correctly; only `image` and `gallery` round-tripped into invented data
- **Builder page order and hierarchy did not survive a sync**: The sidebar arrangement lives in `.order.json`, owned by its own repository rather than being part of the collection, so sync never carried it. The receiving site kept its own ordering, and any page it had no entry for landed alphabetically at the bottom — so a synced page could sit somewhere quite different from where it sits on the source. Parent/child hierarchy did not travel either, living in the same file. The order now rides on the collection's **settings** as `pageOrder`: it is configuration rather than content, so it belongs to the selection an operator already makes when they tick Pages under Collection Settings, and a reorder shows up as the collection differing in the existing diff rather than needing a category of its own. Because collections are imported before objects and the order file reconciles against the pages that currently exist, applying it at collection time would strip every id as unknown — so it is deferred and applied last, after objects land. Export reads through the repository rather than the service, because the service writes an order file when none exists and an export must never mutate the site it is reading. Also fixes the Sync Manager's checkboxes shrinking to nothing as labels grew longer
- **Renaming a folder in a depot renamed the wrong folder**: A depot renders one shared dialog for the whole field and re-constructs a `Dialog` against it per folder, passing a callback that closes over the folder being renamed. `Dialog` caches one instance per DOM node and returned it early, silently discarding the options passed to every later construction — so the first rename worked and every one after it re-ran the original callback, renaming the first folder edited instead of the selected one. Customers saw a nested folder rename itself when they renamed its parent. Nothing looked wrong until the rename landed, because the name field is set on the shared node before opening and pre-filled correctly; the stale callback also compared against the wrong folder's original name, so its no-op guard misfired. The one-instance-per-node behaviour stays, since that is what stops listeners being double-bound, but the cached path now adopts the new options
- **Editing a document stripped obfuscated email addresses back to plaintext**: The styledtext editor only keeps attributes a node or mark declares, and while block types collected every `data-*` into a declared attribute, inline marks silently lost everything except `class`. The obfuscated `mailto` spans are the case that surfaced it — a round-trip through the editor reduced a span carrying `data-user` and `data-domain` to a bare `class="mailto-obfuscated"`, which publishes the address in plaintext *and* leaves the decoder nothing to rebuild the link from, so the result is worse than a plain `mailto` while still looking obfuscated. The four marks that wrap arbitrary inline HTML now collect `data-*` the way blocks already did, plus `title`, which was lost the same way and is the visible tooltip on those same spans. Bold, italic and friends are deliberately excluded: they are toolbar-generated, never carry `data-*`, and giving them attributes would stop adjacent identical marks merging
- **A conventionally organised Site Builder looked empty to two callers**: The template lister defaults to a non-recursive scan, which sees only `.twig` files sitting loose at the top of `builder/` — and builder templates live in `layouts/`, `pages/`, `partials/` and `macros/`, which is exactly what `builder:init` scaffolds. The dashboard's template count therefore read `0` on a site with 35 templates, and the starter scaffold guard, which checks `pages/` before overwriting, saw a site nesting its pages as empty and would let `builder:init` scaffold a starter over an existing site. The guard stays scoped to `pages/` rather than widening to the whole tree, because the built-in defaults layer ships a layout and the lister unions across read layers, so a whole-tree check is non-empty on a fresh install and would refuse every first scaffold
- **An image's content hash changed on writes that did not touch the image**: The hash exists to bust ImageWorks crops when the rendered output could change, and `uploadDate` was fed into it to catch a file replaced in place — a case that cannot actually arise, since every upload gets a unique filename suffix, so `name` already differs whenever new bytes land, a re-upload of an identical file included. It bought no cache-busting and cost stability: any write that rebuilt an untouched image field produced a fresh hash, churning crops for nothing and making two identical images compare unequal across installs, so pages carrying no image at all read as "differs" in the Sync Manager purely because each side stamped its own date. Every existing image's hash changes once on its next write — a one-time cache-bust, spread out as objects are saved
- **The admin's last hardcoded English strings now follow the site locale**: Three separate places ignored the configured language. Every form's Save and Delete buttons were English everywhere, because the label string was doing double duty as a permission flag — `save: canSave ? "Save" : false` — so all 19 call sites had to spell out the English word just to say "show it". Those options now resolve in one place: `true` means the default label, `false` hides the button, and an explicit string still wins, so custom wording is unaffected; a truthy non-string resolves to the default rather than hiding the button, so a permission helper returning `1` cannot silently drop Save. The confirm dialog's Cancel and confirm buttons were hardcoded defaults that nothing ever overrode, even though every call site already localized the message above them. And the depot browser's filter placeholder and Preview button had no route to the translator at all — the form-field layer never received one — which made the depot look half-translated next to its already-localized dialogs; the translator now reaches every field type rather than just the depot. `btn.save` and `btn.delete` were already translated in all seven locales and merely unused; `btn.save_settings` and the wizard's skip link are new across every locale file. A guard test scans the templates and fails if a hardcoded Save or Delete label reappears
- **The collection table's filter ignored list fields**: It only inspected scalar properties, so typing a known tag or category returned nothing. The filter now descends one level into list-shaped arrays and matches each scalar item. Associative composites — image, card, color, file — stay excluded: their internal metadata is not what the operator sees in the table, and matching it would make a term like "png" hit every row
- **Forty broken cross-links in the documentation**: Links written as relative Markdown paths with an extension — `[ID Settings](id.md)` — passed through the docs build untouched, while extensionless paths get rewritten to real routes. On a page served at `/fields/all-fields/`, `id.md` therefore resolved to `/fields/all-fields/id.md`, which is not a route. The reported link turned out to be one of 40 with the same defect across 14 pages: the whole Extensions section cross-linking itself, most of Automations, and several paths between sections. Every one pointed at a page that exists — nothing was missing, they were unreachable purely because of the extension. All rewritten to the form the build understands, fragments preserved, with no `.md` links left anywhere outside internal notes

### Enhanced

- **A stale integrity manifest is now caught before it can be committed**: rc.19 moved this check into CI, but an install tracking `dev-develop` can pull a broken commit before CI ever runs — and that is not hypothetical: a commit editing one adapter shipped without rebuilding `resources/bundle`, and every install that pulled it answered the first write with "installation has been corrupted". A pre-commit hook now runs the same check, about 0.3s over 987 files, and names the fix when it fails. It also warns when unstaged changes are present, because the manifest hashes the working tree rather than the index — rebuilding with unstaged edits records content that is not in the commit, a manifest wrong the moment it lands. Install it with `bin/install-hooks.sh`, which copies hooks file by file rather than setting `core.hooksPath`; that config replaces the hooks directory wholesale and would disable hooks other tools install of their own accord. The installer refuses to overwrite any hook it did not write
- **Rebuilding the reference index over unchanged docs is now a true no-op**: `reference-index.json` carried a `built_at` timestamp, so every rebuild rewrote 412KB of otherwise identical content and showed up as a diff. Nothing read the field — not the admin viewer, not the docs extension's `docs_lookup`, not the tests — and git already records when the file was last built, more reliably than a self-reported stamp. Any diff in that file now means the documentation actually changed, and the release build stops emitting a meaningless one-line change in every release commit

## [3.5.0-rc.19] - 2026-08-16

### Added

- **MCP prompts can target a Data View, not just a collection**: A prompt's **Target** now offers collections and Data Views in one picker, grouped under two headings, and its access inherits from whichever you choose. Data Views were already fully exposed over MCP — `list_views`, `query_view`, `tcms://view/{id}` — but a prompt could only name a collection, so a prompt scoped to a view had to set its access by hand. Worth knowing when you inherit from a view: views deliberately do not participate in access-group filtering, so `authenticated` on a view means *any* signed-in caller, where on a collection it means callers whose groups grant access. If a prompt's reach should respect groups, target a collection or set Access explicitly. An id carried by both a collection and a view resolves to the collection — the stricter of the two — and the old `targetCollection` field keeps working, so nothing needs re-saving

### Changed

- **The MCP server moves from Pro to Standard, along with text watermarks and barcodes**: MCP was Pro-only, which meant most sites never met the feature this release is named for. Standard now includes the endpoint and the anonymous persona, so a Standard site can expose collections for an AI agent to read and can reach the bundled documentation tools. Lite does not include MCP — it stays the free way in, not a way to get the AI surface for nothing. Writing still requires Pro, because both credentials that grant a non-anonymous persona are Pro features: API keys for the admin persona, the OAuth server for the per-user one. That gate is now enforced where the persona is decided rather than only where credentials are issued, which closes a real hole — a key or token can outlive the licence that created it, through a lapsed trial, a downgrade, or a restored backup, and would otherwise have granted full read/write MCP on a licence entitled to anonymous reads only. Such a credential is now treated as absent rather than rejected: the caller falls through to anonymous and still gets whatever is genuinely public. Two things degrade rather than error below Pro — Data Views are Pro, so `list_views` and `query_view` return empty, and custom schemas are Pro, so only built-in schemas are exposed. Separately, **Text Watermarks** and **Barcodes** move from Pro to Standard

- **One name for a collection's configuration**: The same thing — a collection's `.meta.json` — was called "Collection Settings" in the documentation and the admin route, "Collection Meta" in the Sync Manager and the permission matrix, and "Collection Metadata" in several doc headings. The Access Groups screen used two of the three in consecutive lines. Everything an operator reads now says **Collection Settings**, matching the screen that has always lived at `/admin/collections/{id}/settings`; `meta` remains only where it is genuinely technical and should not move — the `.meta.json` filename, the `collectionsMeta` permission key, and the `--collection-meta` CLI flag. The permission matrix now reads "Collection Settings → Collection Objects", which states what the two dimensions actually distinguish. No routes, flags or documentation URLs changed

- **Collections and Data Views are no longer exposed as MCP resources unless you ask**: `mcp.resource` now defaults to off, in the schema *and* at runtime. Previously a collection that had never touched the setting was addressable at `tcms://{collection}/`, its objects at `tcms://{collection}/{id}`, and it appeared in `resources/list` — exposure nobody chose, on every collection, from the moment MCP was switched on. Resources are a second route to data the tools already serve: `get_object`, `query_collection`, `search_collection` and `get_resource` are unaffected, so an agent loses entries in `resources/list`, not the ability to read anything. **A collection that explicitly stored `true` keeps its exposure** — only collections that never set the value change, which is the population that never made a choice about it. Turn it back on per collection under the collection's MCP settings, or per view in the dataviews editor. One knock-on worth knowing: a site where nothing has opted in advertises no `resources` capability at all during the MCP handshake, which is honest rather than a fault — there are no resources to serve

### Fixed

- **A sync push the remote refused reported itself as complete**: The receive endpoint answers `200` even when its importer wrote nothing — it collects per-item failures in the response body rather than throwing — and the sending side built its result from the status code alone, discarding a body that said precisely what went wrong. Pushing a collection whose name is reserved came back as "Push complete." while the remote had already reported "Cannot save collection with a reserved name". Both `push` and `pull` now read the result they were given and surface the remote's own errors, and `tcms push`/`pull` exit non-zero when the far side refused, so a failed deploy is visible to a script instead of passing silently
- **A settings-only sync could not be pulled at all, and reported the wrong thing when pushed**: `pull` decided whether there was anything to do by counting schemas, templates and objects — never collections — so a payload carrying only collection settings short-circuited to "Nothing to pull" and never reached the importer. Separately, the summary counted objects under the collections key in three places (the service, the CLI renderer and the admin template), so a settings sync reported objects it had never touched while showing `0` for the collections it had just created. Collections and objects are now counted and labelled separately everywhere
- **The Sync Manager showed a difference that could never be resolved**: Comparing against production reported "1 only on production" indefinitely. The entry was the Twig Playground's collection, which each install creates on demand the first time someone opens the tool — so it existed on one side and not the other. Because the picker lists local collections, a remote-only entry could not even be selected. Underneath was an asymmetry: the Playground is deliberately excluded from object sync, but its *settings* were exported anyway, so sync mirrored the container while refusing its contents. It is now excluded from both directions and from the picker, including payloads arriving from an older remote
- **Builds could ship a stale documentation index without failing**: `bin/build.sh` stripped dev dependencies and then ran the documentation index builder, which parses `src/` with `nikic/php-parser` — a package that existed only transitively through the test tooling and was removed by that very step. The builder fataled, and because that step alone lacked the error check every other critical step has, the build reported success and copied the previous day's index into the release. The index build now runs before dependencies are stripped, has the missing guard, and `nikic/php-parser` is declared directly so it cannot vanish again
- **A stale integrity manifest is now caught in CI rather than in production**: When `resources/bundle` drifts from the files it describes, an install answers every request with "installation has been corrupted" naming one file — usually not the file that caused it. Nothing caught it: the runtime check is skipped under `APP_ENV=test`, so the test suite passes either way, and the release build regenerates the manifest against `dist`, leaving the committed one untouched. `composer run bundle:check` compares the committed manifest against the tree and lists exactly which files drifted; CI runs it before the build, where it cannot be masked
- **The Algolia Search extension could be enabled on editions that cannot run it**: Its manifest declared no minimum edition, so the admin offered the toggle on Standard even though Algolia search is a Pro feature and its own description says so — enabling it there simply did nothing. It now declares `pro`, matching how the Pushover extension already behaved
- **Sync diff badges were pushed out of view by long labels**: A collection or template with a long name squeezed its drift badge off the row. The badge now keeps its place and the label truncates instead

### Enhanced

- **The core frontend assets are now documented where people build front ends**: `cms.assetsHead()` and `cms.assetsBody()` emit the nine stylesheets and scripts Total CMS ships for its own Twig output — grid, galleries, pagination, icons, htmx, and the decoder behind the `mailto` filter — plus anything an extension registers. They were previously documented only under Extensions, framed as something you do for extension assets, while three pages actively taught the opposite by hand-loading two or three of the nine. A front end built by the book therefore shipped without the grid, gallery and pagination stylesheets, and nothing errored — `{% cmsgrid %}` simply rendered unstyled. The Twig overview now carries the canonical section with a table of what breaks when they are missing, the Site Builder pages show both calls in their layout examples, and the pages that taught hand-loading have been corrected

## [3.5.0-rc.18] - 2026-08-14
...
- **OAuth tokens now inherit the connecting user's access groups, on both MCP and REST**: Before this, every authenticated OAuth connection saw the same flat tier regardless of who approved it — a blogger's token and an admin's token reached the same collections, and a Bearer token's scopes were treated as its entire authority on REST, so a `cms:write`-scoped token could write to *any* collection no matter which group the approving user belonged to. Authorization is now request-time and identity-aware: a token's real reach is the intersection of its consented scopes, the approving user's access-group permissions, and (for MCP) the collection's own AI exposure setting. A blogger who connects Claude can now ask it to draft and save posts in the blog collection they can already edit in the dashboard — `create_object`, `update_object`, and `patch_object` move from admin-only to group-gated, the actual payoff of the feature — while remaining unable to touch anything their groups don't cover. `cms:admin` issuance widens to match: a non-admin user whose groups grant schema, collection-settings, or utility permissions can now request and receive it, scoped to what those groups allow; full elevation to the unrestricted admin persona still requires admin-group membership, unchanged. `/api/cache` gains the same group check it was missing (any logged-in non-admin could previously flush caches and toggle devmode). Drafts follow the same rule: visible only where a caller's groups grant `read`, never from public exposure alone. One deliberate exception — `get_view` / `query_view` and the `tcms://view/{id}` resource check only the view's own `mcp.access` field, not access groups; an `authenticated`-exposed view built over a group-denied collection remains a valid read path, so keep views scoped to `admin` unless their output is safe for the tier you expose them to. MCP has not shipped to production, so this is launch semantics for the AUTHENTICATED tier, not a migration — existing admin-group connections are unaffected; a non-admin user's prior token loses reach it arguably should never have had
- **The admin now shows what AI and OAuth connections can actually reach**: The authorization above is an intersection of three layers — consented scopes, access-group permissions, collection exposure — which makes "what can Claude touch on this site?" genuinely hard to answer by inspection. Three surfaces now answer it. The Permission Matrix gains an AI/MCP dimension per collection: read where the collection is public-exposed or the group grants it, create/update only where the group grants them on a collection exposed to authenticated MCP callers; delete never appears, because MCP ships no delete tool. Collections left at the default (unexposed) `mcp.access` get no column, and a site not using MCP sees no new dimension at all. The OAuth Grants page gains an **Effective reach** row per grant — the computed intersection, collection by collection — because a raw scope badge like `cms:write` can badly overstate what a group-capped grant can do; the computation honors the same exposure gate the write tools enforce, so a collection a group can edit but MCP can't reach is never claimed as writable. And the Access Groups form now says plainly that its permissions also govern connected AI clients and OAuth API connections, not just dashboard logins
...
- **`cache.domainScoped` — one cache for installs sharing one tcms-data**: Cache keys have always been namespaced by domain, so several installs pointed at the same data directory each cached identical content under different names — a save through one install left the others serving stale content for the full TTL. Setting `cache.domainScoped` to `false` namespaces content caches by the data path instead, so every install sharing the folder shares one cache and a save through any of them is seen by all — while unrelated site-groups on a shared Redis pool stay isolated from each other, since the namespace derives from the path rather than disappearing. Identity stays domain-scoped regardless of the setting: each domain carries its own license, so license verdicts, sessions, and password-reset tokens are never shared. In shared mode the filesystem cache moves to `tcms-data/.system/cache` so installs stop answering from private stale files; compiled Twig templates remain per-install. Defaults to `true` — existing installs keep today's exact behavior
...
- **Super admins exist only in the default auth collection**: `isSuperAdmin()` looked an id up in the default auth collection without asking which collection the caller actually belonged to. That distinction only exists on sites running a second auth collection — a members area via `auth.publicRegistration`, say — but there it mattered: a member whose object id collided with an admin's id in the default collection was treated as a super admin, and because the registration endpoint forces `active` and `groups` but accepts a caller-supplied id, that collision could be chosen. The check now takes the caller's collection and refuses outright when it isn't the default one — super admins live in the default auth collection by definition. Every core call site passes the collection: the session paths, the OAuth/MCP paths from the token's own identity, and the Twig `cms.auth.isSuperAdmin()` that gates Site Builder content, the most exposed of them. Sites with a single auth collection — the overwhelming default — were never exploitable, and extension code calling the old signature keeps its exact old behavior
- **"Select Endpoints" API keys couldn't authorize any API route**: The endpoint picker stores its grants without the `/api` prefix — `/sync`, `/collections/blog` — vocabulary from before this release cycle moved every API route under `/api`. The paths being checked carry the prefix, so no picker-created grant could ever match: every "Select Endpoints" key got a 401 on every `/api` route, and the only key that worked was "All Endpoints" — quietly turning a least-privilege feature into all-or-nothing. (MCP, Automations, and XML-RPC grants kept working; those routes don't live under `/api`.) Path matching now treats the `/api` prefix as the routing artifact it is, on both the grant and the request, so existing scoped keys simply start working — including the rc.15 "Sync Manager" option, which never worked at all. Found via a customer who could only get sync running with an all-endpoints key
- **Video embeds mangled their player parameters (YouTube error 153)**: The `embed()` Twig function built YouTube and Vimeo query strings with an already-escaped `&amp;` separator, and the iframe's attribute escaping then escaped it again — every parameter after the first arrived as `amp;loop`, `amp;origin`, and so on. YouTube's player now refuses such embeds outright with error 153 rather than shrugging off the junk parameters, which is how a long-standing bug surfaced as a customer report this cycle. Queries are now escaped exactly once, and source URLs that arrive pre-escaped (`watch?si=x&amp;v=y` from an escaped Twig variable) are normalized before parsing, so both clean and escaped inputs produce a working embed
...
- **Clear failed jobs without discarding pending work**: Errors piled up in the job queue with no practical way to remove them — `Clear Import Queue` deletes everything, so tidying an error list also threw away a half-drained import, and the only alternative was waiting on a timer that (see below) never ran. The Job Queue Manager gains a second button that removes failed jobs only, reports how many it cleared, and leaves pending jobs untouched. Available to scripts as `DELETE /api/jobqueue/status/failed`

### Fixed

- **The license manager showed a blank panel**: Retiring the `license.totalcms.test` domain removed the `licenseUrl` config property, but the license manager template still built its iframe address from it. The Twig `config()` accessor returns an empty string for a property that doesn't exist, so nothing errored — the address simply collapsed to a relative `/store/{domain}` and the iframe pointed back at the customer's own site instead of the license server. The store URL is now derived from the same hardcoded constant the license API calls already use, so there is one place a license host is named and no configuration to fall out of step. Restoring the setting was deliberately not the fix: a configurable license URL is a spoof-server surface, which is why it was removed
- **Failed jobs were never pruned automatically**: A 30-day prune existed but nothing reached it. Its only caller is `tcms jobs:process`, which returns early when there is no pending work — so a queue holding nothing but old failures exited before the prune ran, and the errors stayed indefinitely. Maintenance now runs before the queue is inspected, so it happens on every invocation, and the queue status is reported afterwards and describes what is actually left. Retention also drops from 30 days to 15: a failed job is diagnostic output, not data. Note this still requires `jobs:process` to be on a cron schedule — an unscheduled queue prunes nothing, and the dashboard's stalled-queue warning is the signal that this is the case
- **The dashboard's system-status panel left the license row blank**: The panel reused the sidebar's status logic, which deliberately reports nothing when a license is healthy — correct for a warning icon that should be absent, wrong for a status row, which then rendered empty and read as broken rather than reassuring. A healthy license now names its edition, and a simulated edition says so explicitly, since simulation changes what the site can do and reporting the simulated edition silently would misrepresent the install. Anything needing attention passes through exactly as before, severity and trial countdown intact
- **Re-importing an RSS feed produced a queue full of failed jobs**: Feed entry ids are slugified titles, so polling a feed again regenerated the id of every entry already imported, and each duplicate was queued regardless. Nothing was overwritten — a duplicate id is rejected on save — but each one landed as a failed job, and the reported import count described what was queued rather than what was saved. The waste was larger than the failed jobs: an entry's image is downloaded before the job is queued, so every duplicate refetched its image into the temp directory, and only a job that succeeds consumes that file — a feed with images leaked one temp file per entry per poll. Entries that already exist are now skipped before any extraction or download work, and the returned count reflects only what was queued. Identity is still the id, so an entry that changes its title imports as a new object
- **A wrong-typed field in a schema import returned a 500**: A schema carrying a value the serializer cannot map — a `formgrid` authored as an array of rows rather than a newline-delimited string — escaped `SchemaFactory` as an uncaught `TypeError`, producing a server error and an error-monitoring event instead of a rejection. Those failures now surface as an `InvalidArgumentException` carrying the serializer's own message, which the import action already renders as a 400. Hand-edited schema files fail the same way when read, rather than only on import. Unsupported shapes are still refused — just cleanly, and with a message that says what was wrong
- **Imports of records without an id logged warnings and undercounted**: A record may legally omit `id` — schema autogeneration assigns one at save — but the JSON importer read `$record['id']` unguarded in its exists-check and log lines, so every id-less record raised undefined-array-key warnings. More visibly, the import loop only counted records that carried an id, so autogenerated records imported successfully and then vanished from the reported total. The CSV importer had the identical pattern for files with no id column. Both now guard those reads, log `(autogen id)` where a record has none, and count every record imported; only the created/updated id lists skip them, since those ids cannot be known before the save
...
- **`patch_object` MCP tool — partial updates without the round-trip**: `update_object` is a full replace, which makes it a data-loss hazard for AI agents: changing one field means fetching the complete object (with `format: "html"` so styled text survives), carrying every other field forward faithfully, and stripping the decorated `url` — and any field the agent drops silently reverts to its schema default. The new `patch_object` merges only the fields in the payload over the stored object, through the same `ObjectPatcher` the REST `PATCH` route uses, so omitted fields simply keep their values. Containers (card, deck, list) replace whole rather than deep-merging — sending the complete container is also how an item is removed — clearing a field is explicit (`""` / `[]`), and binary fields can never be written or cleared through a patch. `update_object` remains for genuine whole-record rewrites; its description now points agents at the patch tool for subset edits
- **`tcms schema:lint` — validate schemas edited in place**: `schema:import` validates on the way in, but that assumed imports were how schemas change — AI agents (and operators) edit the JSON files directly, and nothing ever re-checked them until something broke at runtime. The linter runs the import-path checks against stored schemas without saving: no `id` property defined (save-time code auto-adds `id` to `required` without checking the property exists — the failure then blames your data instead of the schema), `required`/`index` naming undefined properties, `inheritFrom` pointing at a missing parent (silently skipped at runtime, losing every inherited property), deck/card `schemaref` targets that don't exist or contain deck-incompatible types, and full meta-schema validation. Warnings flag what errors can't: properties missing help text — which feeds the MCP tool catalog and is the only documentation an AI agent has for the content model — and schemas missing descriptions. Lints all custom schemas by default or one by id, `--strict` promotes warnings to failures, `--json` for CI
- **`tcms object:create` — create one object from the CLI**: The gap every agent hit: `collection:import` wants an array and a file, the admin wants a browser, and creating a single object from a script meant curl against the REST API. The command takes one JSON object from a file or stdin (`echo '{...}' | tcms object:create blog -`) and runs it through the full save pipeline — schema defaults, validation, index update, `object.created` event — exactly as the admin form would. It refuses to overwrite an existing id and points arrays at `collection:import`
- **The agent skill now covers live-site work**: The bundled skill (`.claude/skills/totalcms`, installed by Composer) taught the local build loop but nothing about the two mistakes that actually burn agents on a live site: MCP writes that replace whole objects, and stale local seed files re-imported over live content. Two new references cover editing content through a site's MCP server (tool selection, patch-first writes, the full-replace round-trip, format `"html"`) and the go-live lifecycle (before launch the local project is the source of truth, after launch the server is; exclusive sync filters; dry-run first). The docs gain a matching **Editing with AI Agents** page under MCP, a first-connection troubleshooting table in the MCP server page mapping the symptoms actually hit in the field (DCR off by default, ungenerated OAuth keys, user-agent firewalls, missing `mcp:*` scopes) to their fixes, and a **Source-of-Truth Rule** section in the sync docs

### Fixed

- **Logging in mid-OAuth returns to the consent screen, and the login redirect is same-origin only**: An operator who hit an OAuth authorize link while signed out was bounced to the login page with a `next` parameter nothing ever read — after signing in they landed on the dashboard, stranding the connecting AI client mid-flow. The authorize action now stores the return URL in the session the same way admin pages do, so login leads straight back to consent with the full authorize request intact. Reviewing that code surfaced an open-redirect: the login action accepted an arbitrary `?redirect=` target from a crafted link, a phishing primitive. Post-login redirects are now restricted to local paths and same-host URLs — protocol-relative (`//evil.example`) and non-http schemes included in the rejection — with anything else falling back to the dashboard
- **Translation placeholders substitute correctly when passed by bare name**: The OAuth consent screen greeted operators with "You are signed in as: %admin%" — the translator replaces parameter keys verbatim, so a template passing `{user: id}` against a string containing `%user%` swapped only the letters inside the delimiters. The same bug rendered stray braces (or worse — `'{entries} entries'` became `'100 100'`, the bare key also eating the literal word) across the orphan scanner, builder sidebar, and builder assets pages. The normalization lives in `TranslationService::trans()` — the choke point every route to the translator shares (the global Twig `t()`, `cms.locale.t()`, and PHP callers): bare parameter names map onto both delimiter styles (`{name}` and `%name%`) and are never substituted undelimited; call sites that already pass explicit delimiters are untouched. The locale files themselves are standardized on `{name}` as the house style — every `%name%` placeholder converted across all seven admin locales (the `%` form stays supported for extension strings)
