...
- **A `noindex` page no longer emits a canonical link.** Declaring a canonical URL tells a crawler which address to index, and asking not to be indexed in the same head is a mixed signal — one that search engines are documented as resolving by ignoring the directive. `cms.seo.head()` and `cms.seo.canonical()` now drop `<link rel="canonical">` whenever the record's SEO card has **No Index** on, including when the card names an explicit Canonical URL. `og:url` still prints: it is an identity for a share card, not an instruction to a search engine.
