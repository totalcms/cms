...
final class AdminDocsActionTest extends TestCase
{
	...
	{
		$uri = $this->createMock(UriInterface::class);
		$uri->method('getPath')->willReturn('/admin/docs/api');
		$uri->method('getQuery')->willReturn('');
		...
		$this->renderer->expects($this->once())
			...
			->with(
				$this->response,
				'admin/docs.twig',
				$this->callback(fn ($data): bool => isset($data['page']) && $data['page'] === 'api')
			)
			->willReturn($expectedResponse);

		$result = ($this->action)($this->request, $this->response, ['page' => 'api']);
