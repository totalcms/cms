...
it('describes each parameter from the schema help, or its label', function (): void {
	$html = ($this->render)("{{ webmcp_form('blog') }}");

	$titleHelp = (string)$this->schemas->fetchSchemaForCollection('blog')->properties['title']['help'];

	expect(webmcpInput($html, 'title'))->toContain('toolparamdescription="' . htmlspecialchars($titleHelp, ENT_QUOTES) . '"');
});
...
it('strips markup from a help text before it becomes a parameter description', function (): void {
	...
	expect(webmcpInput($html, 'body'))->toContain('toolparamdescription="Say something nice"')
		->and(webmcpInput($html, 'mood'))->toContain('toolparamdescription="Mood"');
});
...
it('never puts object data into a tool string', function (): void {
	...
	// attributes and never reach toolname / tooldescription / toolparamdescription.
	$c = $this->app->getContainer();
	...
	preg_match_all('/tool(?:name|description|paramdescription)="([^"]*)"/', $html, $m);
