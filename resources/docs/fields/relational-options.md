...
Instead of referencing a collection directly, you can populate options from a [DataView](docs/collections/data-views). Use the `view` key instead of `collection`. The `view` and `collection` settings are mutually exclusive.
