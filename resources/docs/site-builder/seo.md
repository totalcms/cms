...
- **Description.** A mapped property is as often markdown as it is HTML, so both are flattened: tags are stripped, entities decoded, then the markdown markers are removed — a link or an image keeps its text and loses its brackets and URL, `**bold**`, `_italic_` and `` `code` `` lose their wrappers, and a heading marker, a list bullet or a `>` at the start of a line goes. Ordinary prose comes back untouched: a lone `!`, a `$5 * 3` or a `my_var` is a sentence, not markdown, and is left alone. Whatever survives is then collapsed to single spaces. That cleanup is for mapped properties, which hold body copy — a description typed into the SEO card is taken as written, markers and line breaks included. The 160-character truncation is the one step that applies to every description: card, mapped property or site default, whichever won, is cut on a word boundary with an ellipsis.
- **Social title.** `og:title` always prints: the card's **Social Title** when it has one, otherwise the record's own title before the site title template is applied — the bare `About`, not `About | Bistro` — falling back to the site name on a page with no record behind it. `twitter:title` is different: it is emitted **only** when the card carries a Social Title, because Twitter reads `og:title` when there is no `twitter:title` and a second identical tag buys nothing. There is no collection mapping for the Social Title — it is a per-record override or nothing.
...
| `description` | The resolved, cleaned, truncated description |
