...
The exception is `totalcms/docs` (see the table below), which ships marked `default_enabled` in its manifest — see [`default_enabled`](docs/extensions/manifest#default-enabled) for the mechanism and why it's bundled-only.
...
| `totalcms/docs` | Exposes this install's own documentation to AI agents as `docs_search`, `docs_get`, and `docs_lookup` MCP tools. Enabled by default (see below); tools stay `authenticated`-only unless you opt in to public exposure. | [Documentation Tools →](docs/mcp/docs-tools) |
...
| `totalcms/podcast` | Host a podcast from two collections: the `podcast` and `podcast-episode` schemas, a feed the directories accept at its own address, and a `podcast_feed()` Twig function. Standard edition and above. | [Podcast →](docs/extensions/podcast) |
