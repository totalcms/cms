# Total CMS Documentation

Welcome to the Total CMS documentation! This comprehensive guide will help you get the most out of your content management system.

## Quick Start

New to Total CMS? Start here:

- **[Dashboard Overview](dashboard.md)** - Learn your way around the admin interface
- **[Configuration](configuration.md)** - Set up your system correctly
- **[Twig Overview](twig-overview.md)** - Master the templating system

## Core Documentation

### System & Configuration
- **[Configuration](configuration.md)** - Complete configuration reference
- **[Data Model](data-model.md)** - Understanding collections, objects, and properties
- **[Field Settings](field-settings.md)** - Configure form fields and validation
- **[Field Options](field-options.md)** - Field-specific configuration options

### Content Management
- **[Dashboard Overview](dashboard.md)** - Admin interface guide
- **[Import Data](import.md)** - Importing content from external sources
- **[Export Data](export.md)** - Backing up and exporting your content
- **[Download Files](download.md)** - File download and media management

### Development & Integration

#### Templating (Twig)
- **[Twig Overview](twig-overview.md)** - Complete Twig integration guide
- **[Twig Filters](twig-filters.md)** - Available filters and their usage
- **[Twig Functions](twig-functions.md)** - Built-in template functions
- **[Twig Forms](twig-forms.md)** - Form helpers and generation
- **[TotalCMS Factory](twig-factory.md)** - Factory pattern implementation
- **[GET Data](twig-get-data.md)** - Accessing GET parameters
- **[POST Data](twig-post-data.md)** - Working with POST data
- **[Session Data](twig-session-data.md)** - Session management in templates
- **[QR Codes](twig-qrcodes.md)** - Generating QR codes

#### APIs & Integration
- **[REST API](rest-api.md)** - RESTful API endpoints and usage
- **[PHP API](php-api.md)** - Using Total CMS in PHP applications
- **[Search](search.md)** - Implementing search functionality

### System Administration
- **[Authentication](auth.md)** - User management and authentication
- **[Licenses](licenses.md)** - License management and validation
- **[Templates](templates.md)** - Template system overview
- **[Filesystem](filesystem.md)** - File and directory management
- **[Factory Pattern](factory.md)** - Factory pattern implementation

## Video Tutorials

For visual learners, check out our video series:

### Latest Overview (Beta 11)
Learn about the newest features and improvements:
[Total CMS 3 Beta 11 Overview](https://www.youtube.com/watch?v=jjGdyTniJT4)

### Complete Tutorial (Beta 6)
Full walkthrough from installation to deployment:
[Total CMS 3 Beta 6 - Full Tutorial](https://www.youtube.com/watch?v=jFvGecshJVk)

### Additional Resources
- [Beta 10 Overview](https://www.youtube.com/watch?v=cM50y4o2I9w)
- [Beta 7 Overview](https://www.youtube.com/watch?v=R-bmaJLAHjw)

## Common Tasks

### For Content Creators
1. **Creating Content**: Start with the [Dashboard Overview](dashboard.md)
2. **Managing Media**: See [Download Files](download.md)
3. **Organizing Content**: Learn about the [Data Model](data-model.md)
4. **Bulk Operations**: Check [Import](import.md) and [Export](export.md) guides

### For Developers
1. **Template Development**: Begin with [Twig Overview](twig-overview.md)
2. **API Integration**: Read the [REST API](rest-api.md) and [PHP API](php-api.md) docs
3. **Custom Fields**: Study [Field Settings](field-settings.md)
4. **Search Implementation**: Follow the [Search](search.md) guide

### For System Administrators
1. **Initial Setup**: Review [Configuration](configuration.md)
2. **User Management**: See [Authentication](auth.md)
3. **System Security**: Check [Licenses](licenses.md)
4. **File Management**: Learn about [Filesystem](filesystem.md)

## Getting Help

### Documentation Structure
- **Guides**: Step-by-step tutorials for common tasks
- **Reference**: Complete documentation of features and APIs
- **Examples**: Code samples and real-world implementations
- **Troubleshooting**: Solutions to common problems

### Additional Resources
- **GitHub Repository**: Access source code and report issues
- **Community Forum**: Connect with other Total CMS users
- **Support Tickets**: Get direct help from the development team
- **Video Tutorials**: Visual learning resources

### Contributing to Documentation
Found an error or want to improve the documentation? We welcome contributions:

1. Documentation files are located in `resources/docs/`
2. Use Markdown format for consistency
3. Include code examples where helpful
4. Test all code samples before submitting
5. Follow the existing style and structure

## API Reference

For developers integrating with Total CMS:

- **[REST API Documentation](rest-api.md)** - HTTP API endpoints
- **[PHP API Documentation](php-api.md)** - PHP integration examples
- **[Twig API Documentation](twig-functions.md)** - Template functions reference

## What's New

Total CMS 3 introduces major improvements:

- **Modern Architecture**: Built with Slim 4, PHP 8.2+, and modern design patterns
- **Enhanced Security**: CSRF protection, input sanitization, and secure sessions
- **Improved Performance**: Optimized file handling and caching
- **Better Developer Experience**: Comprehensive APIs and documentation
- **Flexible Configuration**: Environment-based configuration system

## Browser Support

Total CMS admin interface supports:
- Chrome 90+
- Firefox 88+
- Safari 14+
- Edge 90+

## System Requirements

- PHP 8.2 or higher
- Web server with URL rewriting
- Write permissions for data directories
- Composer for dependency management

---

**Need immediate help?** Check the troubleshooting section in relevant documentation pages or contact support through the admin panel.

*This documentation is continuously updated. Last updated: December 2024*