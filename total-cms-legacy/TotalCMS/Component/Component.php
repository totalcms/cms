...
abstract class Component implements ComponentInterface
{
	...
	public function __construct($slug,$options=array())
	{
		...
		// if(preg_match(':[^a-zA-Z0-9\p{L}-_\$]/:u',$slug)) {
	...
	public function urlify_string($string)
	{
		...
		return strtolower(preg_replace("/\s+/","-",trim(preg_replace("/[\[\]{}\/\\\`~|&!%#$+=\"\':;><.,?()*^]/u",'',urldecode($string)))));
	}
    ...
    public function backup($prefix=false)
    {
    	if (!file_exists($this->target_path())) return false;
    ...
    public function save_content($contents,$options=array())
    {
    	...
    	if (!$this->make_dir($this->target_dir)) return false;
