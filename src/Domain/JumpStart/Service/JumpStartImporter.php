...
final class JumpStartImporter
{
	 ...
	 */
	private function generateFactoryObjectData(string $collectionId, string $objectId, array $objectData): array
	{
		...
		foreach ($objectData as $property => $value) {
			if ( is_string($value)
				&& (str_starts_with($value, 'image') || str_starts_with($value, 'gallery'))
				&& $this->factoryImporter->isFakerRule($value)
			) {
