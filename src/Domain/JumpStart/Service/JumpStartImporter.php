	...
	private function processObjects(array $objects): void
	{
		foreach ($objects as $objectDef) {
			...
			try {
				...
				foreach ($objectDef['data'] as $property => $value) {
					if (is_string($value) && FactoryImporter::isFakerRule($value)) {
				...
				// Merge static data on top of generated data
				$finalData = array_merge($generatedData, $staticData);

				// Save the object
				$this->objectSaver->saveObject($collectionId, $finalData);
