...
/**
 * The gallery helpers behind `cms.render.gallery()`, `galleryLauncher()`,
 * `galleryImage()`, `galleryAlt()` and `galleryCaption()`: the lightGallery
 * grid, the programmatic launcher template, one image out of a gallery, and
 * the alt/caption text (including `{alt} — {exif.camera}` caption templates,
 * rendered through a private, dependency-free Twig environment).
 *
...
class GalleryRenderer
{
	...
	public function gallery(string|array|null $idOrObject, array $thumbSettings = [], array $fullSettings = [], array $options = []): string
	{
		...
		foreach ($images as $image) {
			...
			$img = HTMLUtils::inlineElement('img', [
				...
				'alt'           => ImageRenderer::altFromImageData($image),
		...
		if ($maxVisible > 0) {
			...
			if ($viewAllText !== null) {
				// HTMLUtils::element() escapes attribute values; escaping here too
				// double-encoded the text ("See &amp;lt;all&amp;gt;").
				$attributes['data-view-all-text'] = (string)$viewAllText;
			}
		}
	...
	public function galleryAlt(string|array $idOrObject, string|int $name, array $options = []): string
	{
		...
		return ImageRenderer::altFromImageData($image);
	}
