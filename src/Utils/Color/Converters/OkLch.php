...
abstract class OkLch
{
	...
	public static function oklchToHex(array $oklch): string
	{
		...
		if (!$rgb instanceof \TotalCMS\Utils\Color\Colors\Rgb) {
	...
	public static function oklchChange(array $oklch, array $change): array
	{
		...
		if (!$oklchColor instanceof \TotalCMS\Utils\Color\Colors\OkLch) {
		...
		$oklchColor = $oklchColor->change(
			...
			chroma: $change['c'] ?? null // Don't let ColorFactory handle hue changes
		);

		$coordinates = array_map(fn (float|int|string $c): float => round((float)$c, 3), $oklchColor->coordinates());
	 ...
	 */
	public static function cleanEnhanced(mixed $value, ?bool $throw = null): array
	{
