...
use TotalCMS\Utils\Color\Converters\OkLch;
use TotalCMS\Utils\Color\Converters\Rgb;
use TotalCMS\Utils\Color\Exceptions\CallbackDoesNotExist;
use TotalCMS\Utils\Color\Exceptions\UnknownColorSpace;
use TotalCMS\Utils\Color\Exceptions\UnsupportedCoordinateModifier;
...
abstract class Util
{
	 ...
	 * any change.
	 *
	 * @param  \Stringable|string|int|float $value The value to transform
	 *
	 * @return \Stringable|string|int|float        The transformed or unchanged value
	 */
	public static function addLeadingZero(
		\Stringable|string|int|float $value,
	): \Stringable|string|int|float {
		return (self::isStringable($value) && \str_starts_with((string)$value, '.'))
			? "0$value"
			: $value
		;
	}
	 ...
	 *
	 * @param  \Stringable|string|int|float      $value The original coordinate you want to replace or update
	 * @param  \Stringable|string|int|float|null $new   Either a new value which will be returned instead of the original $value, or a modifier string which will be used to update $value
	 * @param  bool                               $hex   If true $value and $new will be considered as strings containing hexadecimal numbers
	 * @param  bool                               $throw If false the function will not throw exceptions
	 * @param  bool                               $hue   If true applies 360° wraparound for hue values
	 */
	public static function changeCoordinate(
		\Stringable|string|int|float $value,
		...
		if ($hue && !$hex && ($result < 0 || $result >= 360)) {
	 ...
	 *
	 * @param \Stringable|string|int|float $value     The stringable or number to clean
	 ...
	 * @param int|null                     $precision Used to round the value
	 * @param bool                          $round     If true the value will be rounded
	 */
	public static function cleanCoordinate(
		\Stringable|string|int|float $value,
		?float $min       = 0,
		?float $max       = 100,
		bool $loop      = false,
	 ...
	 *
	 * @param  int|null $precision A potentially invalid number of decimals
	 *
	 * @return int                 The corresponding valid number of decimals
	 */
	public static function cleanPrecision(
		?int $precision = null,
	): int {
		...
		return \min($precision ?? 0, $default);
	}
	 ...
	 * @param  int            $length    The minimum length expected
	 * @param  bool|null       $uppercase If true $value will be converted to uppercase,
	 ...
	 */
	public static function cleanHexValue(
	 ...
	 *
	 * This fonction only works with color spaces handled by the TotalCMS\Utils\Color\ColorSpace enum.
	 *
	 ...
	 * @param bool|null    $throw    If $value does not correspond to any supported color space and this parameter
	 ...
	 */
	public static function findColorSpace(
		mixed $value,
		?ColorSpace $fallback = null,
		?bool $throw    = null,
	): ?ColorSpace {
		$throw ??= !$fallback;
	 ...
	 */
	public static function hexToDec(
		\Stringable|string $value,
	): float {
		return (float)\hexdec(
	 ...
	 */
	public static function isColorString(
		mixed $value,
	 ...
	 *
	 * @return bool        True if $value is an array or an instance of \Traversable, false otherwise
	 */
	 ...
	 *
	 * @return bool        True if $value is a string or a \Stringable object, false otherwise
	 */
	...
	public static function multiplyMatrices(
		...
		// transpose $b:
		$bCols = \array_map(
			callback : fn (int|string $k): array => \array_map(
				fn (array $i) => $i[$k],
				$b,
			),
		...
		$product = \array_map(
			callback : fn ($row): array => \array_map(
				callback : fn (array $col): mixed => \is_array($row)
					? \array_reduce(
						array    : $row,
						callback : fn ($a, $v, $i = null): float|int => $a + $v * (
							$col[$i ?? \array_search($v, $row)] ?? 0
						),
						initial  : 0,
					) : \array_reduce(
						array    : $col,
						callback : fn ($a, $v): float|int => $a + $v * $row,
						initial  : 0,
					),
				array : $bCols,
			),
		...
		if ($p === 1) {
			// Avoid [[a], [b], [c], ...]]:
			return \array_map(
				callback : fn ($v): float|int => \is_array($v) ? $v[0] : $v,
				array    : $product,
			);
		}
	...
	public static function parseColorValue(
		...
		return self::isStringable($value)
			? self::parseStringColorValue($value, $opacityFactor)
			: self::toArray($value)
		;
	}
	...
	public static function startsWith(
		...
		foreach (self::toIterable($needles) as $needle) {
	...
	public static function toIterable(
		...
		return self::isIterable($value)
			? $value
			: self::toArray($value, $keep)
		;
	}
	...
	public static function toColor(
		...
		$values = self::toArray($cleanFrom($value, $throw));
	 ...
	 */
	public static function toColorSpace(
	 ...
	 *
	 * @return bool          True if $value is an array and if all of its values are validated using $filter, false otherwise
	 */
